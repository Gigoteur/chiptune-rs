 ...
 */
KLYSAPI extern ChiptuneSong * KSND_LoadSong(ChiptunePlayer* player, const char *path);
KLYSAPI extern ChiptuneSound * KSND_LoadSound(ChiptunePlayer* player, const char *path);
 ...
 * @return @c ChiptuneSong or @c NULL if there was an error.
 */
KLYSAPI extern ChiptuneSong * KSND_LoadSongFromMemory(ChiptunePlayer* player, void *data, int data_size);
 ...
 */
KLYSAPI extern void KSND_FreeSong(ChiptuneSong *song);

/**
 * Get song length measured in pattern rows
 */
KLYSAPI extern int KSND_GetSongLength(const ChiptuneSong *song);
 ...
 *
 * If @c song is freed using KSND_FreeSong(), the corresponding @c ChiptuneSongInfo may be 
 ...
 */
KLYSAPI extern const ChiptuneSongInfo * KSND_GetSongInfo(ChiptuneSong *song, ChiptuneSongInfo *data);
 ...
 */
KLYSAPI extern int KSND_GetPlayTime(ChiptuneSong *song, int position);

/**
 * Create a @c ChiptunePlayer context and playback thread.
 *
 * After setting a @c ChiptuneSong to be played with KSND_PlaySong() the song will start playing
 * automatically.
 *
 * @param sample_rate sample rate in Hz
 * @return @c ChiptunePlayer context or @c NULL if there was an error
 */
KLYSAPI extern ChiptunePlayer* KSND_CreatePlayer(int sample_rate);
 ...
 * @return @c ChiptunePlayer context or @c NULL if there was an error
 */
KLYSAPI extern ChiptunePlayer* KSND_CreatePlayerUnregistered(int sample_rate);

/**
 * Free a @c ChiptunePlayer context and associated memory.
 */
KLYSAPI extern void KSND_FreePlayer(ChiptunePlayer *player);

/**
 * Start playing song @c song from position @c start_position (measured in pattern rows)
 */
KLYSAPI extern void KSND_PlaySong(ChiptunePlayer *player, ChiptuneSong *song, int start_position);

/**
 * Stop playback on a player context.
 */
KLYSAPI extern void KSND_Stop(ChiptunePlayer* player);
 ...
 *
 */
KLYSAPI extern void KSND_Pause(ChiptunePlayer *player, int state);

/**
 * Fill a buffer of 16-bit signed integers with audio. Data will be interleaved for stereo audio.
 *
 * Use only with a @c ChiptunePlayer context created with KSND_CreatePlayerUnregistered().
 *
 * @param player player context
 ...
 */
KLYSAPI extern int KSND_FillBuffer(ChiptunePlayer *player, short int *buffer, int buffer_length);
 ...
 */
KLYSAPI extern void KSND_SetPlayerQuality(ChiptunePlayer *player, int oversample);
 ...
 */
KLYSAPI extern void KSND_SetVolume(ChiptunePlayer *player, int volume);
 ...
 */
KLYSAPI extern void KSND_SetLooping(ChiptunePlayer *player, int looping);
 ...
 */
KLYSAPI extern int KSND_GetPlayPosition(ChiptunePlayer* player);
 ...
 */
KLYSAPI extern void KSND_GetVUMeters(ChiptunePlayer *player, int *envelope, int n_channels);
